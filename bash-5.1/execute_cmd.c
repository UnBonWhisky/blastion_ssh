...
// Permet de ne pas afficher ces commandes la
bool is_filtered(const char *command) {
  const char *filtered_commands[] = {
    "alias", "export", "eval", "unset", ".", "test", "dircolors", "shopt", "/usr/bin/locale-check", "groups", "lesspipe", "/usr/bin/clear_console", "/usr/local/libexec/sftp-server"
  };
  const int num_filtered_commands = sizeof(filtered_commands) / sizeof(filtered_commands[0]);

  for (int i = 0; i < num_filtered_commands; i++) {
    if (strcmp(command, filtered_commands[i]) == 0) {
      return true;
    }
  }
  return false;
...
// Permet d'ajouter dans le fichier /var/log/auth.log le PTS et la commande tapee via les droits du user syslog
void log_command(WORD_LIST *words) {
  if (words) {
    if (words->word->word[0] != '[' && !is_filtered(words->word->word)) {
      char *username = getenv("USER");
      if (username && strcmp(username, "root") != 0) {
        char command[4096] = "logger -p auth.info -t bash_5.1.0-custom '[custom_bash] PTS: ";

        FILE *tty_output = popen("tty", "r");
        if (tty_output) {
          char tty_name[256];
          fgets(tty_name, sizeof(tty_name), tty_output);
          strtok(tty_name, "\n");
          strcat(command, tty_name);
          pclose(tty_output);
        }
